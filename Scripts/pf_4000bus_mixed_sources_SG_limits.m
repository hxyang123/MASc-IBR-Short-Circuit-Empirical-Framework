...
%   5 Vset (pu), 6 Qmin, 7 Qmax, 8 Imax, 9 Pmin, 10 Pmax
%
% Bus 1 is an external slack/grid bus and is not counted as an SG. This is
% necessary because a slack source must remain free to balance total P and Q.
[bus, branch, dev] = createDemoCase( ...
    nbus, PminSG, PmaxSG, QminSG, QmaxSG, ImaxIBR);
...
[VmPre, VaPre, PcalcPre, QcalcPre, infoPre] = sparsePowerFlowNR( ...
    ...
    deg2rad(bus(:,4)), pfTol, pfMaxIter, true, dev, bus);
...
% Verify synchronous-generator limits
sgRows = find(dev(:,2) == 2);
Psg = zeros(numel(sgRows),1);
Qsg = zeros(numel(sgRows),1);
for k = 1:numel(sgRows)
    row = sgRows(k);
    b = dev(row,1);
    Psg(k) = Pcalc(b) + bus(b,5);
    Qsg(k) = Qcalc(b) + bus(b,6);
end

sgLimitTol = 1e-6;
if any(Psg < dev(sgRows,9)-sgLimitTol | Psg > dev(sgRows,10)+sgLimitTol)
    warning('At least one SG violates its active-power limit.');
end
if any(Qsg < dev(sgRows,6)-sgLimitTol | Qsg > dev(sgRows,7)+sgLimitTol)
    warning('At least one SG violates its reactive-power limit.');
end
...
PdevReport = zeros(size(dev,1),1);
QdevReport = zeros(size(dev,1),1);
for k = 1:size(dev,1)
    b = dev(k,1);
    if dev(k,2) == 1
        local = find(ibrRows == k, 1);
        PdevReport(k) = Pibr(local);
        QdevReport(k) = Qibr(local);
    else
        PdevReport(k) = Pcalc(b) + bus(b,5);
        QdevReport(k) = Qcalc(b) + bus(b,6);
    end
end

resultDev = table(dev(:,1), dev(:,2), PdevReport, QdevReport, ...
    ImagDev, rad2deg(IangDev), dev(:,9), dev(:,10), dev(:,6), dev(:,7), ...
    'VariableNames', {'Bus','DeviceType','P_pu','Q_pu','I_pu', ...
    'CurrentAngle_deg','Pmin_pu','Pmax_pu','Qmin_pu','Qmax_pu'});
...
% ------------------------------------------------------------------------
function [bus, branch, dev] = createDemoCase( ...
    nbus, PminSG, PmaxSG, QminSG, QmaxSG, ImaxIBR)
...
% Replace this function with readtable/readmatrix for a real network.
    ...
    dev = zeros(nIBR+nSG,10);

    % IBRs: PQ-controlled in prefault power flow. Pmin/Pmax are unused.
    for k = 1:nIBR
        dev(k,:) = [ibrBus(k) 1 0.006 0.001 1.0 ...
            -0.005 0.005 ImaxIBR -Inf Inf];
    end

    % SGs: PV buses with explicit P and Q limits of [-2, 2] pu.
    for k = 1:nSG
        dev(nIBR+k,:) = [sgBus(k) 2 0.012 0.0 1.0 ...
            QminSG QmaxSG 2.0 PminSG PmaxSG];
    end
end
...
function [Pspec, Qspec, busType, Vset] = assembleInjections(bus, dev)
    ...
    for k = 1:size(dev,1)
        ...
        if dev(k,2) == 2
            % Clamp every synchronous-generator active-power schedule.
            Pg = min(max(dev(k,3), dev(k,9)), dev(k,10));
            Qg = min(max(dev(k,4), dev(k,6)), dev(k,7));
        else
            Pg = dev(k,3);
            Qg = dev(k,4);
        end
...
function [Vm, Va, Pcalc, Qcalc, info] = sparsePowerFlowNR( ...
    Ybus, Pspec, Qspec, busType, Vset, Va0, tol, maxIter, enforceQlim, dev, bus)
    ...
    for iter = 1:maxIter
        ...
        if enforceQlim
            ...
            for kk = 1:numel(sgRows)
                ...
                % Generator output equals net bus injection plus local load.
                Qgen = Qcalc(b) + bus(b,6);
                if Qgen > dev(row,7)
                    Qspec(b) = dev(row,7) - bus(b,6);
                    busType(b) = 1;
                elseif Qgen < dev(row,6)
                    Qspec(b) = dev(row,6) - bus(b,6);
                    busType(b) = 1;
                end
            end
        end
