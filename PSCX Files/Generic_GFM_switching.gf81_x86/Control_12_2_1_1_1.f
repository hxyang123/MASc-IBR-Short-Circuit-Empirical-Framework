...
!=======================================================================
      ...
      SUBROUTINE Control_12_2_1_1_1Dyn(Iabc, Vabc, I_ref_dq, freq, phi, &
     &   Qref, Vref, Pref, Vpeak, Ibase, Imax, Pmax, Pmin, Qmax, Qmin,  &
     &   Kp_lim, Ti_lim, w_base, w_type, Kppll, Kipll_T, Kpp, Kip_T,    &
     &   Kdrp, Kpv, Kiv_T, Kpvq, Kivq, Kpq, Kiq_T, DVS_dbd, DVS_gain,   &
     &   v_drp, Kp_rocof, Ti_rocof, Qflag, Vflag, PQflag, Vdip, Vup,    &
     &   T_frz, K_pod, Tw_pod, T1_pod, T2_pod, vd_Kp, vd_Ki_T, d_vsm,   &
     &   m_vsm, Cfilter_pu, Dblk)
...
!---------------------------------------
      ...
      IF ( TIMEZERO ) THEN
         FILENAME = 'Control_12_2_1_1_1.dta'
         CALL EMTDC_OPENFILE
         SECTION = 'DATADSD:'
...
! 730:[abcdq61_2_1_1_1] abc dq0 transformation 
...
!  BUTTERWORTH FILTER
!
      CALL COMPONENT_ID(ICALL_NO,1893159916)
...
! 830:[abcdq60_2_1_1_1] abc dq0 transformation 
!ABC to DQ0 transformation
...
!  BUTTERWORTH FILTER
!
      CALL COMPONENT_ID(ICALL_NO,726491360)
...
! 880:[EPRI_PLL_22_2_1_1_1]  
      CALL EPRI_PLL_22_2_1_1_1Dyn(Freq_pll_pu, phis, Vpvabc, REAL(IT_1),&
     & RT_1, Eref, Vref, RT_2, w_type, Dblk, 75.0, Kppll, Kipll_T, w_bas&
     &e, Kdrp, Kp_rocof, Ti_rocof, v_drp, d_vsm, m_vsm, T_frz, Pmax, Pmi&
     &n, Qmax, Qmin, Kp_lim, Ti_lim)
...
!  BUTTERWORTH FILTER
!
      CALL COMPONENT_ID(ICALL_NO,802269439)
...
!  BUTTERWORTH FILTER
!
      CALL COMPONENT_ID(ICALL_NO,988325438)
...
! 1020:[sqrt] Square Root 
      ...
      IF (RT_74 .LT. 0.0) THEN
        RT_43 = 0.0
        IF (.NOT. LVD1_1) THEN
          CALL EMTDC_MESSAGE(ICALL_NO,1202547182,1,2,"A negative value i&
     &s detected as an input to the Square Root function. ")
          CALL EMTDC_MESSAGE(ICALL_NO,1202547182,1,-1,"Input is treated &
     &as 0.0.")
          STORL(NSTORL) = .TRUE.
        ENDIF
      ELSE
...
! 1030:[div_171_1_2_1_1_1] Divider 
!
...
! 1080:[deadband_28_1_1] Deadband with offset and gain 
      RT_50 = 0.0
...
!  Hard_Limit
      ...
      ELSE
        ...
        IF (STORL(NSTORL) .EQV. .FALSE.) THEN
          CALL EMTDC_WARN(ICALL_NO,1274370776,1,"Lower limit is more tha&
...
! 1490:[sqrt] Square Root 
      ...
      IF (RT_88 .LT. 0.0) THEN
        RT_85 = 0.0
        IF (.NOT. LVD1_1) THEN
          CALL EMTDC_MESSAGE(ICALL_NO,122820631,1,2,"A negative value is&
     & detected as an input to the Square Root function. ")
          CALL EMTDC_MESSAGE(ICALL_NO,122820631,1,-1,"Input is treated a&
     &s 0.0.")
          STORL(NSTORL) = .TRUE.
        ENDIF
      ELSE
...
!  Hard_Limit
      ...
      ELSE
        Ipvq_ref = RT_84
        IF (STORL(NSTORL) .EQV. .FALSE.) THEN
          CALL EMTDC_WARN(ICALL_NO,114361575,1,"Lower limit is more than&
...
! 1550:[sqrt] Square Root 
      ...
      IF (RT_91 .LT. 0.0) THEN
        RT_86 = 0.0
        IF (.NOT. LVD1_1) THEN
          CALL EMTDC_MESSAGE(ICALL_NO,402678399,1,2,"A negative value is&
     & detected as an input to the Square Root function. ")
          CALL EMTDC_MESSAGE(ICALL_NO,402678399,1,-1,"Input is treated a&
     &s 0.0.")
          STORL(NSTORL) = .TRUE.
        ENDIF
      ELSE
...
! 1590:[gain_50_2_1_1_1] Gain Block 
!  Gain
...
! 1620:[div_171_1_2_1_1_1] Divider 
!
...
! 1630:[gain_50_2_1_1_1] Gain Block 
!  Gain
      phi = 1.0 * phis
...
!=======================================================================
      ...
      SUBROUTINE Control_12_2_1_1_1Out()
...
!=======================================================================
      ...
      SUBROUTINE Control_12_2_1_1_1Dyn_Begin(DVS_dbd, DVS_gain, T_frz)
...
!     SUBR    EPRI_PLL_22_2_1_1_1Dyn_Begin  ! 
...
! 880:[EPRI_PLL_22_2_1_1_1]  
      CALL EPRI_PLL_22_2_1_1_1Dyn_Begin(T_frz)
...
!=======================================================================
      ...
      SUBROUTINE Control_12_2_1_1_1Out_Begin(DVS_dbd, DVS_gain, T_frz)
...
!     SUBR    EPRI_PLL_22_2_1_1_1Out_Begin  ! 
...
! 880:[EPRI_PLL_22_2_1_1_1]  
      CALL EPRI_PLL_22_2_1_1_1Out_Begin(T_frz)
